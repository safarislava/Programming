...
public class MyProgram {
    public static float CalculateCell(short[] n, float[] x, int i, int j) {
        if (n[i] == 13) {
            ...
            float a = (float) Math.pow(Math.E, Math.pow(2/3 * x[j], 3));
            return (float) Math.exp(Math.pow(Math.cos(a), 2));
        }
        else {
            float a = (float) Math.pow(4 / Math.tan(x[j]), 2), 
                b = (float) Math.pow(Math.sin(x[j]), Math.pow(2/3 * x[j], x[j]) / 3) - 1/2;
